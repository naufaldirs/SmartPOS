...
function addRow(event) {
  ...
  var newRow = '<tr data-kd-sparepart="' + kdSparepart + '">' +
      ...
      '<td class="namaSparepart">' + namaSparepart + '</td>' +
      '<td class="hargaSparepart">' + hargaSparepart + '</td>' +
      '<td><input type="text" name="qty[]" class="qty-input" style="width:25%;" oninput="calculateSubtotal(this)"></td>' +
      '<td><input type="number" name="subtotal[]" readonly style="width:50%;" class="subtotal-input"> </td>' +
      '<td><a href="#" class="btn btn-danger btn-lg active delete-row" role="button" aria-pressed="true" onclick="deleteRow(this)"><i class="fa fa-minus-square" style="color:red"></i></a></td>' +
      '</tr>';
...
function calculateSubtotal(input) {
    ...
    var harga = parseInt($(input).closest('tr').find('.hargaSparepart').text());
    var subtotal = isNaN(qty) ? 0 : qty * harga;
    $(input).closest('tr').find('.subtotal-input').val(subtotal);
    calculateTotal();
}
